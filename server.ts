...
async function getFaceColors(uuid: string): Promise<string[]> {
  ...
  if (!skinRes.ok) throw new Error("Failed to fetch skin image");
  const skinBytes = new Uint8Array(await skinRes.arrayBuffer());
  const decoded = decode(skinBytes);
  if (!decoded || !decoded.pixels) {
    throw new Error("Failed to decode skin PNG or pixels missing");
  }
